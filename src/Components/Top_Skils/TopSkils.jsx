...
function TopSkils() {
  ...
  const cards = [
    {
      img: card1,
      button: "304k designer",
      title: "User experience designers",
      category: "Design & Creative",
    },
    {
      img: card3,
      button: "210k developers",
    ...
    {
      img: card3,
      button: "180k marketers",
      title: "Digital Marketers",
      category: "Sales & Marketing",
    },
    {
      img: card4,
      button: "120k translators",
      title: "Translators",
      category: "Writing & Translation",
    },
  ];
    ...
    <div className="max-w-[1240px] mx-auto mb-[140px] pt-[80px]">
      ...
      <div className="mt-6">
        <div className="block sm:hidden">
          <select value={active || ""} onChange={(e) => setActive(e.target.value || null)} className="block mx-auto w-[85%] max-w-[260px] border border-gray-300 rounded-md px-2.5 py-1.5 text-[13px] sm:text-[14px] bg-white shadow-sm focus:outline-none focus:ring-2 focus:ring-[#D22F26] focus:border-[#D22F26]">
        ...
        <div className="hidden sm:flex justify-center gap-4 sm:gap-6 md:gap-10 text-[14px] sm:text-[16px] md:text-[18px] font-medium">
          {tabs.map((tab) => (
            <button key={tab} onClick={() => setActive(active === tab ? null : tab)} className="relative pb-2 group">
              <span className={`transition-colors duration-300 ${active === tab ? "text-[#D22F26]" : "text-[#6F6F6F] group-hover:text-[#D22F26]"}`}>{tab} </span>
              <span className={`absolute left-0 right-0 -bottom-[1px] h-[2px] transition-all duration-300 ${active === tab ? "bg-[#D22F26]" : "bg-transparent group-hover:bg-[#D22F26]"}`}></span>
            </button>
          ))}
        </div>
      </div>
      ...
      <div className="mt-[61px] relative">
        ...
        >
          {cards
            .filter((card) => !active || card.category === active)
            .map((card, index) => (
              <SwiperSlide key={index}>
                <div className="relative bg-[#253D9C] w-[285px] h-[235px] rounded-[18px] p-5 cursor-pointer shadow-md pt-[129px] mx-auto">
                  <img src={card.img} alt="" className="transition-transform duration-300 hover:-translate-y-2 absolute top-[-160px]" />
                  <button className="text-[12px] text-white bg-[#3859BF] px-4 rounded-[4px]">{card.button}</button>
