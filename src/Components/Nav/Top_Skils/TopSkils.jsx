    ...
    <div className="max-w-[1240px] mx-auto pt-[80px]">
      ...
      <div className="mt-6">
        <div className="flex sm:hidden overflow-x-auto whitespace-nowrap px-2 scrollbar-hide">
          {tabs.map((tab) => (
            <button key={tab} onClick={() => setActive(tab)} className="inline-block relative pb-2 group mr-6 last:mr-0 flex-shrink-0">
              <span className={`transition-colors duration-300 ${active === tab ? "text-[#D22F26]" : "text-[#6F6F6F] group-hover:text-[#D22F26]"}`}>
                {tab}
              </span>
        ...
        <div className="hidden sm:flex justify-center gap-6 md:gap-10 text-[14px] sm:text-[16px] md:text-[18px] font-medium">
          {tabs.map((tab) => (
            <button key={tab} onClick={() => setActive(tab)} className="relative pb-2 group">
              <span className={`transition-colors duration-300 ${active === tab ? "text-[#D22F26]" : "text-[#6F6F6F] group-hover:text-[#D22F26]"}`}>
                {tab}
              </span>
              <span className={`absolute left-0 right-0 -bottom-[1px] h-[2px] transition-all duration-300 ${active === tab ? "bg-[#D22F26]" : "bg-transparent group-hover:bg-[#D22F26]"}`}></span>
